...
func (m *Repository) Reservation(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		log.Println("can not get reservation from session")
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(err)
		m.App.Session.Put(r.Context(), "error", "can not insert reservation into db")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		log.Println(err)
		m.App.Session.Put(r.Context(), "error", "can not insert room restriction into db")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	htmlMessage := fmt.Sprintf(
		`
			...
			Dear %s, <br>
			This is confirmation of your reservation from %s to %s for room %s.
		`,
	...
	htmlMessage = fmt.Sprintf(
		`
			...
			Dear owner, <br>
			A reservation for room %s has been made from %s to %s by %s %s.
		`,
		reservation.Room.RoomName,
		reservation.StartDate.Format("2006-01-02"),
		reservation.EndDate.Format("2006-01-02"),
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println(err)
		m.App.Session.Put(r.Context(), "error", "can't parse startDate!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		log.Println(err)
		m.App.Session.Put(r.Context(), "error", "can't parse endDate!")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
	...
	if err != nil {
		log.Println(err)
		m.App.Session.Put(r.Context(), "error", "can not get availability in db")
		http.Redirect(w, r, "/", http.StatusTemporaryRedirect)
		return
	}
...
// BookRoom takes URL prams, builds a sessional variable and take user to make res page
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	...
	id, _, err := m.DB.Auntheticate(email, password)
	if err != nil {
		log.Println(err)
...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	// assume that there is no month/year specify
	now := time.Now()
	...
	for _, x := range rooms {
		...
		// get all the restriction for the current time
...
// AdminPostShowReservation update the showed reservation in the admin tool
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// get the block map from the session. Loop through entire map, if there's an entry in the map
		// that doesnt exist in posted data, and restriction id > 0, then it is a block we need to remove
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				...
				// the rest are just placeholder for days without block
				if val > 0 {
