...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	query := `
			update users 
			set first_name = $1, last_name = $2, email = $3, access_level = $4,
			updated_at = $5
			where id = $6
			;`
...
func (m *postgresDBRepo) Auntheticate(email, testPassword string) (int, string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	query := `
			select id, coalesce(reservation_id, 0), restriction_id, room_id,
			start_date, end_date
			from room_restrictions
			where
				$1 < end_date
...
func (m *postgresDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	_, err := m.DB.ExecContext(ctx, query,
		startDate,
		startDate.Add(time.Hour*23), // because using AddDate(0,0,1) will include the next day
		id,
		2,
		time.Now(),
		time.Now(),
