...
func TestForm_Required(t *testing.T) {
	...
	form.Required("a", "b", "c")
	if !form.Valid() {
		t.Error("form shows does not have required field when it does")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("shows minLength of 3 is not met when data is longer")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("email")
	if !form.Valid() {
		t.Error("form shows invalid for correct email")
	}
	...
	form.IsEmail("email")
	if form.Valid() {
		t.Error("form shows valid for incorrect email")
	}
}
